...
void LinuxSockReadFunc()
{
        while (SocketActivated) {
                //Read UDP socket with OpenTrack data
                memset(&OpenTrack, 0, sizeof(OpenTrack));
                bytes_read = recvfrom(socketS, (char*)(&OpenTrack), sizeof(OpenTrack), 0, (struct sockaddr*)&from, &fromlen);
...
EVRInitError CServerDriver_Sample::Init( vr::IVRDriverContext *pDriverContext )
{
        ...
        //Open UDP port for receive data from OpenTrack ("UDP over network", 127.0.0.1, S_PORT)
        int socketS, iResult;
        ...
        int flags = fcntl(socketS, F_GETFL, 0);
        fcntl(socketS, F_SETFL, flags | O_NONBLOCK);
        ...
        if (socketS != -1) {
                ...
                if (iResult != -1) {
                        ...
                        pSocketThread = new std::thread(LinuxSockReadFunc);
                }
                else {
                        close(socketS);
